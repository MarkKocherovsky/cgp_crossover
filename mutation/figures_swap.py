...
cgp_base_path = "../output/swap/"
cgp_base = []
cgp_base_p_fits = []
cgp_1x_path = "../output/cgp_swapone/"
cgp_1x = []
cgp_1x_p_fits = []
#cgp_2x_path = "../output/swap_twopoints/"
#cgp_2x = []
#cgp_2x_p_fits = []
color_order = ['blue', 'lightgreen', 'red']
method_names = ["CGP_Swap", "CGP_Swap_one"]
method_names_long = ["CGP_Swap", "CGP_Swap_one"]
max_e = 50
...
for n in range(len(f_name)):
    boxes = axs[n].boxplot([cgp_base_data['p_fits'][n], cgp_1x_data['p_fits'][n]],
...
plt.show()
plt.savefig("../output/figures/fit_swap8.png")
...
for n in range(len(f_name)):
    axs[n].set_yscale('log')
    axs[n].plot(cgp_avgs[n], label='CGP_Swap', c=color_order[0])
    axs[n].plot(cgp_1x_avgs[n], label='CGP_Swap_OnePoint', c=color_order[1])
    #axs[n].plot(cgp_2x_avgs[n], label='CGP_Swap_TwoPoint', c=color_order[2])
    alpha = 0.05
...
plt.show()
plt.savefig("../output/figures/fitness_swap10.png")
