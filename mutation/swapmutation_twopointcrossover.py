...
def two_point_xover(individual1, individual2):
    if random.random() > crossover_prob:
        ...
        i = np.array(range(1, len(input1) - 1))
        index = np.sort(random.choice(i, 2, replace=False))
        front1 = input1[:index[0]].copy()
        mid1 = input1[index[0]:index[1]].copy()
        back1 = input1[index[1]:].copy()
        front2 = input2[:index[0]].copy()
        mid2 = input2[index[0]:index[1]].copy()
        back2 = input2[index[1]:].copy()
        input1 = np.concatenate((front1, mid2, back1))
        input2 = np.concatenate((front2, mid1, back2))
        if output1.shape[0] == 1:
            a = output1.copy()
            output1 = output2.copy()
            output2 = a
        else:
            i = random.randint(0, output2.shape[0])
            front1 = output1[:i].copy()
            back1 = output1[i:].copy()
            front2 = output2[:i].copy()
            back2 = output2[i:].copy()
            output1 = np.concatenate((front1, back2))
            output2 = np.concatenate((front2, back1))
...
Path(f"../output/swapmutation_twopointcrossover/{func_name}/log/").mkdir(parents=True, exist_ok=True)
import pickle
...
for g in range(1, generations + 1):
    ...
    individual1, individual2, best_fit_id, parent2_id = tournament_elitism_selection(parents, fitnesses,
                                                                                     tournament_size=4)
    children1, children2 = two_point_xover(individual1, individual2)
    offspring1 = swap_mutation(children1)
    offspring2 = swap_mutation(children2)
    parents = replacement(parents, offspring1, offspring2)
    generation.append(g)
    fit2.append(fitnesses[best])
best_individual = fitnesses[best]
with open(f"../output/swapmutation_twopointcrossover/{func_name}/log/output_{t}.pkl", "wb") as f:
    pickle.dump(fit2, f)
