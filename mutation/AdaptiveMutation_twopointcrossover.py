...
def adaptive_mutation(individual, fitnesses):
    ...
    f_ind = fitness(individual, y_real, x)
...
Path(f"../output/cgp_adaptive2x/{func_name}/log/").mkdir(parents=True, exist_ok=True)
import pickle
...
with open(f"../output/cgp_adaptive2x/{func_name}/log/output_{t}.pkl", "wb") as f:
    pickle.dump(fit2, f)
