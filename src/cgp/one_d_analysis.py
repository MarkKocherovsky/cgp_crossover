...
crossover_methods = {
    ...
    'N-Point 1d': Method('n_point_1d', 'CGP(40+40)-1x1d', 'CGP(40+40) - One Point 1D', 'darkgreen'),
    'Uniform': Method('uniform', 'cgp(40+40)-Ux', 'CGP(40+40) - Uniform', 'pink'),
    'Uniform 1d': Method('uniform_1d', 'cgp(40+40)-Ux1d', 'CGP(40+40) - Uniform 1D', 'deeppink'),
}
...
for selection in (selection_methods.keys()):
    analyzer.plot_box_plots(selection, metrics['Minimum Fitness'], f'minimum_fitness_{selection}_1d_box_graph',
                             'Fitness of Best Models', 'Crossover Methods', 'min_fitness', log=True, violin=True)
    for metric in list(metrics.values()):
        analyzer.plot_line_graph(selection, metric, f'{metric.full_name.lower().replace(" ", "_")}_{selection}_1d_graph',
