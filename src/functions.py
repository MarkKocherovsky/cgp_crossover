...
class Sphere(MultivariateFunction):
    def __init__(self, dimensions: int, n_points:int = 20):
        super().__init__(dimensions, start=-5, end=5, n_points=n_points, label=f"Sphere_{dimensions}D")
...
class Ackley(MultivariateFunction):
    def __init__(self, dimensions: int = 1, n_points:int = 40):
        super().__init__(dimensions, start=-32.768, end=32.769, n_points=n_points, label=f"Ackley_{dimensions}D")
...
class Rastrigin(MultivariateFunction):
    def __init__(self, dimensions: int = 1):
        ...
        def func(xs: np.ndarray):
            ...
            assert xs.ndim == self.dim, f'len(xs) != self.dim, {len(xs)} != {self.dim}'
...
class Diabetes():
    def __init__(self):
        ...
        self.train_x, self.test_x, self.train_y, self.test_y = train_test_split(x, y, test_size = 0.3, random_state = 42) #let's try having it be 42 each time
    def __call__(self):
        return self.train_x, self.test_x, self.train_y, self.test_y

class Collection:
    def __init__(self, f_set=None):
        if f_set is None:
            ...
            self.func_list = [Koza1, Koza2, Koza3, Nguyen4, Nguyen5, Nguyen6, Nguyen7, Rastrigin, Ackley, Levy, Griewank]
        else:
            raise KeyError(f'No such option as {f_set}, please use \'standard\' or leave the option blank')
    ...
    def __call__(self, index: int, dims: int):
        ...
        if isinstance(func_type, type) and issubclass(func_type, MultivariateFunction):
            ...
            # It's a callable instance (e.g., a function or object with a __call__ method)
            func = func_type()
        else:
