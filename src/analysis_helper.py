...
class AnalysisToolkit:
    def __init__(self, crossover_methods: dict, selection_methods: dict, path: str, problem_list: dict, metrics: list, trials: int,
    ...
    def _load_trial_data(self, problem: str, xover: str, selection_key: str) -> list:
        ...
        for trial in range(self.trials):
            ...
            if data.shape[0] != self.max_generations:
    ...
    def plot_line_graph(self, selection_method: str, metric: str, graph_filename:str, title: str, x_label: str, y_label: str, log:bool = False):
        ...
        fig.legend(list(legend_dict.values()), list(legend_dict.keys()), loc='upper center', ncol=3, bbox_to_anchor=(0.5, 0.96), fontsize=10)
    ...
    def get_median_end_values(self, selection_method:str = 'elite_tournament', save_path:str = 'median_end_fitnesses.csv', metric='min_fitnesses'):
        results = {}
        ...
        for problem_key, problem_name in self.problems.items():
            ...
            for xover_key, xover_obj in self.crossover_methods.items():
                ...
                try:
                    ...
                    print(f"Error loading data for {problem_key} {xover}: {e}")
                    row[xover] = np.nan  # Use NaN to indicate missing data
            results[problem_name] = row

        df = pd.DataFrame.from_dict(results, orient='index')
        #df_t = df.transpose(copy=True)
        df.index.name = 'Problem'
        # Step 1: Rank per problem
        rankings = df.rank(axis=1, method='average', ascending=True)

        # Step 2: Compute mean rank per method
        mean_ranks = rankings.mean(axis=0)
        median_ranks = rankings.median(axis=0)

        # Step 3: Build the final DataFrame correctly
        final_ranking_df = pd.DataFrame({
            'Operator': mean_ranks.index,
            'Mean Rank': mean_ranks.values,
            'Median Rank': median_ranks.values
        })

        # Optionally sort
        final_ranking_df = final_ranking_df.sort_values(by='Mean Rank')
        nemenyi = sp.posthoc_nemenyi_friedman(rankings.values)

        # Optional: assign column names
        nemenyi.index = rankings.columns
        nemenyi.columns = rankings.columns

        print(nemenyi)  # pairwise p-values
        nemenyi.to_csv(f'{save_path}_rank_confidence.csv')
        print(final_ranking_df)

        final_ranking_df.to_csv(f'{save_path}_rankings.csv')
        df.to_csv(save_path)
        

        print(f"✅ Saved median values to: {os.path.abspath(save_path)}")
        return df
    ...
    def get_significance_tables(self, selection_method = 'elite_tournament', output_dir='../output/graphs/significance/'):
        ...
        for problem_key, problem_name in self.problems.items():
            ...
            colors = ['green', '#f08080', '#cd5c5c', '#b22222', '#a52a2a', '#800000', '#000000']  # green + maroon shades
    ...
    def plot_box_plots_compare_old_new(self, old_methods, old_problems, selection_method: str, metric: Metric, graph_filename: str, title: str,
            ...
            for xover_key, xover_obj in self.crossover_methods.items():
                ...
                try:
                    if metric.code_name == 'best_sizes':
                        old_path = Path(f"../output/intermediate_results_old/intermediate_results_{old_problem}_{old_method}_size_old.csv")
                    else:
                        old_path = Path(f"../output/intermediate_results_old/intermediate_results_{old_problem}_{old_method}_old.csv")
