...
def macromicro_mutation(individuals, max_c, max_r, max_d, bank, inputs = 1, n_bias = 10, arity = 2, p_mut = 1/40): #either removes, changes, or adds an instruction
	children = []
	newInstructionGenerator = lgpParentGenerator(1, 1, max_d, bank)
...
def lgp_1pc_mut(p, max_c, max_r, max_d, bank, inputs = 1, n_bias = 10, arity = 2, p_mut = 1/40): #either removes, changes, or adds an instruction
	children = []
	newInstructionGenerator = lgpParentGenerator(1, 1, max_d, bank)
	while len(children) < max_c:
		parent = p.copy()
