...
def get_fit(ind, fitness_dict):
    ...
    :return: Saved metric
    """
    if is_in_dict(ind, fitness_dict):
        return fitness_dict[get_code(ind)]
    raise KeyError(f"Individual {get_code(ind)} not found in fitness dictionary.")


def get_code(ind):
    code = ''.join([str(i)+'_' for i in np.array(ind[0]).flatten()])
    try:
        if isinstance(ind[1], np.ndarray):
           code += ''.join([str(i)+'_' for i in ind[1]])
        else:
           code += ''.join(str(ind[1]))
    except IndexError:
        raise IndexError(f'{ind}')
    return code
...
def save_fitness(ind, val, fitness_dict):
    ...
    """
    code = get_code(ind)
    fitness_dict[code] = val[0]

def get_fitness(ind, fitness_dict):
    """
    :param fitness_dict:
    :param ind: GA individual to save
    :param val: metric to save
...
def choose_from_competition(competition, fitness_dict):
    ...
    """
    fitnesses = [get_fitness(ind, fitness_dict) for ind in competition]
    return tuple(list(competition[np.argmin(fitnesses)]).copy())
...
def tournament_selection(individuals, how_many_to_select, tournament_size, fitness_dict):
    ...
    tournaments = [[individuals[idx] for idx in tournament] for tournament in tournament_indexes]

    selected_individuals = [choose_from_competition(tournament, fitness_dict) for tournament in
                            tournaments]

    return deepcopy(selected_individuals)
