...
def select(pop, f_list, max_p = max_p, n_con = 4):
	...
	idxs = []
	#keep best ind
	#print('Fitnesses given tos election')
	#print(np.round(f_list, 4))
	best_f_i = np.argmin(f_list)
	idxs.append(best_f_i)
	new_p.append(pop[best_f_i])
	new_f.append(f_list[best_f_i])
	while len(new_p) < max_p:
		...
		new_f.append(f_list[w_id])
	#print("New Fitnesses")
	new_f = np.array(new_f)
	#print(np.round(new_f, 4))
	return new_p, idxs
final_fit = []
fit_track = []
...
#fig.legend()
Path(f"../output/cgp_2x/{func_name}/retention/").mkdir(parents=True, exist_ok=True)
plt.savefig(f"../output/cgp_2x/{func_name}/retention/retention_{t}.png")
...
ax.set_xlim(0, max_g)
fig.tight_layout()
Path(f"../output/cgp_2x/{func_name}/change_hists/").mkdir(parents=True, exist_ok=True)
plt.savefig(f"../output/cgp_2x/{func_name}/change_hists/change_hists{t}.png")
...
#fig.legend()
plt.tight_layout()
Path(f"../output/cgp_2x/{func_name}/change_plot/").mkdir(parents=True, exist_ok=True)
plt.savefig(f"../output/cgp_2x/{func_name}/change_plot/change_{t}.png")
...
ax.set_xlabel("Generations")
Path(f"../output/cgp_2x/{func_name}/proportion_plot/").mkdir(parents=True, exist_ok=True)
plt.savefig(f"../output/cgp_2x/{func_name}/proportion_plot/proportion_plot_{t}.png")
