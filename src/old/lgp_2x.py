...
def xover(parents, max_r = max_r, p_xov = p_xov): # 2 point crossover
	children = []
	retention = []
...
Path(f"../output/lgp_2x/{func_name}/log/").mkdir(parents=True, exist_ok=True)
import pickle
fig, ax = plt.subplots()
...
ax.legend()
Path(f"../output/lgp_2x/{func_name}/scatter/").mkdir(parents=True, exist_ok=True)
plt.savefig(f"../output/lgp_2x/{func_name}/scatter/comp_{t}.png")
print(f"../output/lgp_2x/{func_name}/scatter/comp_{t}.png")

fig, ax = plt.subplots()
ax.plot(fit_track)
ax.set_title(f"{func_name} Trial {t}")
Path(f"../output/lgp_2x/{func_name}/plot/").mkdir(parents=True, exist_ok=True)
plt.savefig(f"../output/lgp_2x/{func_name}/plot/plot_{t}.png")
...
fig.legend()
Path(f"../output/lgp_2x/{func_name}/retention/").mkdir(parents=True, exist_ok=True)
plt.savefig(f"../output/lgp_2x/{func_name}/retention/retention_{t}.png")
...
fig.legend()
plt.tight_layout()
Path(f"../output/lgp_2x/{func_name}/change_plot/").mkdir(parents=True, exist_ok=True)
plt.savefig(f"../output/lgp_2x/{func_name}/change_plot/change_{t}.png")
...
ax.set_xlabel("Generations")
Path(f"../output/lgp_2x/{func_name}/proportion_plot/").mkdir(parents=True, exist_ok=True)
plt.savefig(f"../output/lgp_2x/{func_name}/proportion_plot/proportion_plot_{t}.png")
...
def print_individual(ind, a, b, fb_node = first_body_node):
        ...
        with open(f"../output/lgp_2x/{func_name}/best_program/best_{t}.txt", 'a') as f:
                f.write('R\tOp\tI\n')
        print("R\tOp\tI")
...
Path(f"../output/lgp_2x/{func_name}/full_graphs/").mkdir(parents=True, exist_ok=True)
...
dot.render(f"../output/lgp_2x/{func_name}/full_graphs/graph_{t}", view=False)


with open(f"../output/lgp_2x/{func_name}/log/output_{t}.pkl", "wb") as f:
	pickle.dump(bias, f)
