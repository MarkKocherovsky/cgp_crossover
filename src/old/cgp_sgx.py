...
pop = parents + children
#[fitnesses[:, 0], alignment[:, 0], alignment[:, 1]] =  np.array([fitness(train_x_bias, train_y, ind[0], ind[1]) for ind in pop])
print(np.array([fitness(train_x_bias, train_y, ind[0], ind[1]) for ind in pop]))
print(f"Trial {t}: Best Fitness = {best_fit}")
#print("All Fitnesses")
#print(np.round(fitnesses, 5))
#print('Best Fit Over Time')
...
try:
	...
	ax.plot(ret_avg_list, c='blue')
	ax.fill_between(range(ret_avg_list.shape[0]), (ret_avg_list-ret_std_list), (ret_avg_list+ret_std_list), color = 'blue', alpha = 0.1)
ax.set_title(f'{func_name} Trial {t}')
ax.set_ylabel('Average Similarity between Best Parents and Best Children')
ax.set_xlabel('Generations')
#fig.legend()
Path(f"../output/cgp_sgx/{func_name}/retention/").mkdir(parents=True, exist_ok=True)
plt.savefig(f"../output/cgp_sgx/{func_name}/retention/retention_{t}.png")
...
ax.set_xlim(0, max_g)
fig.tight_layout()
Path(f"../output/cgp_sgx/{func_name}/change_hists/").mkdir(parents=True, exist_ok=True)
plt.savefig(f"../output/cgp_sgx/{func_name}/change_hists/change_hists{t}.png")
...
try:
	...
	ax.plot(avg_change_list, c = 'blue')
	ax.fill_between(range(avg_change_list.shape[0]), (avg_change_list-std_change_list), (avg_change_list+std_change_list), color = 'blue', alpha = 0.1)
ax.set_title(f'{func_name} Trial {t}')
ax.set_ylabel('Average(Fitness(Best Child) - Fitness(Best Parent))')
ax.set_xlabel('Generations')
#fig.legend()
plt.tight_layout()
Path(f"../output/cgp_sgx/{func_name}/change_plot/").mkdir(parents=True, exist_ok=True)
plt.savefig(f"../output/cgp_sgx/{func_name}/change_plot/change_{t}.png")
...
ax.set_xlabel("Generations")
Path(f"../output/cgp_sgx/{func_name}/proportion_plot/").mkdir(parents=True, exist_ok=True)
plt.savefig(f"../output/cgp_sgx/{func_name}/proportion_plot/proportion_plot_{t}.png")
