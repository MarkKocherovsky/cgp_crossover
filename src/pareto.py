...
method_handles = {}
color_list = ['blue', 'green', 'deeppink', 'orange', 'red', 'lightcoral', 'grey', 'olivedrab', 'purple', 'dodgerblue',
              'chartreuse', 'indigo', 'peru', 'gold', 'tomato']
...
print('fitness index dtype:', fitness.index.dtype)
print('sizes   index dtype:', sizes.index.dtype)
print('missing in sizes:', fitness.index.difference(sizes.index).tolist()[:5])
print('missing in fitness:', sizes.index.difference(fitness.index).tolist()[:5])
# Plot each problem
for idx, (problem, ax) in enumerate(zip(fitness.index, axes.flatten())):
    ...
    for j, method in enumerate(fitness.columns):
        ...
        fit_data = pd.read_csv(
            f'../output/intermediate_results/{problem.replace(" ", "")}_1d_{method}_{selection}_min_fitnesses.csv',
            header=None)
        size_data = pd.read_csv(
            f'../output/intermediate_results/{problem.replace(" ", "")}_1d_{method}_{selection}_best_sizes.csv',
            header=None)
