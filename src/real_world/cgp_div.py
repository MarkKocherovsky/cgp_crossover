...
try:
    ...
    print(xover_method)
    run_name = f'cgp_diversity_{xover_method}_real_world'
except IndexError:
    print('cgp_dnc.py:\t unknown Crossover Type')
    xover = 'OnePoint'
    run_name = 'cgp_diversity_OnePoint_real_world'
...
def are_individuals_unique(individuals):
    seen = set()
    for ind in individuals:
        # Convert body and output arrays to tuples of their flattened versions
        body_tuple = tuple(ind[0].flatten())
        output_tuple = tuple(ind[1])
        # Check if the individual is unique by looking at the combined tuple
        combined = (body_tuple, output_tuple)
        if combined in seen:
            return False  # Found a duplicate
        seen.add(combined)
    return True  # All individuals are unique
...
for g in range(1, max_g + 1):
    children, retention, d_distro = xover(deepcopy(parents), max_n, first_body_node, method=xover_method, shape_override=xov_override, n_inputs=dims)
...
fitnesses, alignment = processFitness(fitness_objects, test_x_bias, test_y, pop, max_p, max_c, arity, opt = 0)
best_i, best_fit, best_pop, mut_list, mut_cum, xov_list, xov_cum, density_distro, mut_Density_distro, preds, p_a, p_b = processAndPrintResults(
    t, fitnesses, pop, mut_impact, density_distro, mut_density_distro, train_x_bias=test_x_bias, train_y=test_y, mode='cgp')
