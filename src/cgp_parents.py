...
def generate_parents(max_p, max_n=64, bank=(add, sub, mul, div), inputs=1, n_constants=10, outputs=1, arity=2, fixed_length=True):
    parents = []
...
def generate_individual_base(n, arity, first_body_node, bank_len):
    ind_base = np.zeros((int(n), int(arity + 1)), dtype=np.int32)
    for i in range(n):
...
def generate_single_instruction(idx, first_body_node, arity, bank_len=4):
