...
crossover_methods = {
    ...
    'Canonical Full': Method('None/full', 'CGP(1+4)-Full', 'CGP(1+4)-F', 'lightblue', 'solid'),
    ...
    'N-Point Full': Method('n_point/full', 'CGP(40+40)-1x-Full', 'CGP(40+40) - One Point - Full', 'lightgreen',
                           'solid'),
    'N-Point 1D': Method('n_point_1d', 'CGP(40+40)-1x1D', 'CGP(40+40) - One Point 1D', 'limegreen', 'dotted'),
    'N-Point 1D Full': Method('n_point_1d/full', 'CGP(40+40)-1x1D-Full', 'CGP(40+40) - One Point 1D - Full',
                              'darkgreen', 'dotted'),
    'Uniform': Method('uniform', 'CGP(40+40)-Ux', 'CGP(40+40) - Uniform', 'deeppink', 'solid'),
    'Uniform Full': Method('uniform/full', 'CGP(40+40)-Ux-Full', 'CGP(40+40) - Uniform - Full', 'pink', 'solid'),
    'Uniform 1D': Method('uniform_1d', 'CGP(40+40)-Ux1D', 'CGP(40+40) - Uniform 1D', 'hotpink', 'dotted'),
    'Uniform 1D Full': Method('uniform_1d/full', 'CGP(40+40)-Ux1D-Full', 'CGP(40+40) - Uniform 1D - Full', 'indigo',
                              'dotted'),
    'Subgraph': Method('subgraph', 'CGP(40+40)-SGx', 'CGP(40+40) - Subgraph', 'orange', 'solid'),
    'Subgraph Full': Method('subgraph/full', 'CGP(40+40)-SGx-Full', 'CGP(40+40) - Subgraph - full', 'redorange',
                            'solid'),
...
metrics = {
    ...
    'Minimum Test Fitness': Metric('min_test_fitness', 'Minimum Test Fitness', r"\mathrm{Median}(\min(\mathrm{f}))",
                                   True),
    'Median Test Fitness': Metric('med_test_fitness', 'Median Test Fitness',
                                  r"\mathrm{Median}(\mathrm{Median}(\mathrm{f}))", True),
    'Best Model Size': Metric('best_model_size', 'Best Model Size', r"\mathrm{Median}(\mathrm{Best Model Size})",
                              False),
    'Median Model Size': Metric('median_model_size', 'Median Model Size',
                                r"\mathrm{Median}(\mathrm{Median Model Size})", False),
    'Semantic Diversity': Metric('semantic_diversity', 'Semantic Diversity',
                                 r"\mathrm{Median}(\mathrm{Semantic Diversity})", False),
}
...
analyzer = AnalysisToolkit(crossover_methods, selection_methods, base_path, problems, metrics, 50, 6001,
                           output_format='.pdf')
analyzer.compile_averages([0, 2, 5, 7, 10, 13, 16], restart=False, mutation='')
for selection in (selection_methods.keys()):
    analyzer.plot_box_plots(selection, best_test_fitnesses, f'minimum_test_fitness_{selection}_box_graph',
                            'Fitness of Best Models - Test Set', 'Crossover Methods', 'Best Fitness', log=True,
                            violin=False, jitter=True)
    analyzer.plot_box_plots(selection, best_fitnesses, f'minimum_fitness_{selection}_box_graph',
                            'Fitness of Best Models', 'Crossover Methods', 'Best Fitness', log=True, violin=False,
                            jitter=True)
    analyzer.plot_box_plots(selection, best_sizes, f'best_sizes_{selection}_box_graph',
                            'Number of Active Nodes in Best Models', 'Crossover Methods', 'Active Nodes', log=False,
                            violin=False, jitter=True)
    ...
    analyzer.get_median_end_values(metric='min_test_fitnesses',
                                   save_path=f'../output/graphs/median_end_test_fitnesses_{selection}.csv')
    ...
    for metric in list(metrics.values()):
        analyzer.plot_line_graph(selection, metric, f'{metric.full_name.lower().replace(" ", "_")}_{selection}_graph',
                                 f'{metric.full_name} Over Generations', 'Generations', rf'${metric.short_name}$',
                                 log=metric.log)
"""
old_problem_names = {
    #'Koza1_1d': 'Koza 1',
