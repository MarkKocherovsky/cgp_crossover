...
c = Collection()
f_list = c.func_list[:7]
f_name = c.name_list[:7]
...
# color_order = ['blue', 'royalblue', 'skyblue', 'orange', 'lightgreen', 'steelblue', 'mediumseagreen', 'indigo',
# 'green']#, 'cadetblue', 'olive']
...
method_names = ["CGP(1+4)", "CGP(16+64)", "CGP-1x(40+40)", "LGP-1x(40+40)", "CGP-2x(40+40)", "LGP-2x(40+40)",
                "CGP-SGx(40+40)", "LGP-Ux(40+40)"]  #, "CGP-Nx(40+40)", "LGP-Fx(40+40)"]
...
method_names_long = ["CGP(1+4)", "CGP(16+64)", "CGP-OnePoint(40+40)", "LGP-OnePoint(40+40)", "CGP-TwoPoint(40+40)",
                     "LGP-TwoPoint(40+40)", "CGP-Subgraph(40+40)",
                     "LGP-Uniform(40+40)"]  #, "CGP-NodeOnePoint(40+40)", "LGP-FlattenedOnePoint(40+40)"]
...
def get_logs_cgp(base_path, max_e=max_e, f_name=f_name):
    ...
    try:
        ...
        return [logs, np.array(full_fits), np.array(fit_tracks), np.array(active_nodes), np.array(node_prop),
                np.array(avg_chg), np.array(avg_ret), np.array(p_sz_li), hist_li]
    except:
        print(drif_li)
...
def dataDict(data):
    ...
    return {'logs': data[0], 'p_fits': data[1], 'fit_track': data[2], 'nodes': data[3], 'prop': data[4],
            'average_change': data[5], 'average_retention': data[6], 'p_size': data[7], 'histograms': data[8]}
...
for n in range(len(f_name)):
    #print(n), cgp_vlen_data['p_fits'][n]
    boxes = axs[n].boxplot(
        [cgp_base_data['p_fits'][n], cgp_40_data['p_fits'][n], cgp_1x_data['p_fits'][n], cgp_vlen_data['p_fits'][n],
         lgp_1x_data['p_fits'][n], cgp_2x_data['p_fits'][n], lgp_2x_data['p_fits'][n], cgp_sgx_data['p_fits'][n],
         lgp_base_data['p_fits'][n]], showfliers=False, patch_artist=True)
...
axs.flat[-1].set_visible(
    False)  #https://stackoverflow.com/questions/44980658/remove-the-extra-plot-in-the-matplotlib-subplot
...
lgp_1x_4regs_path = "../output/lgp_1x4_regs/"
lgp_1x_2regs_path = "../output/lgp_1x2_regs/"
lgp_1x_2regv_path = "../output/lgp_1x2_regs_var/"
lgp_1x_0regv_path = "../output/lgp_1x0_regs_var/"
lgp_1x_0regs_path = "../output/lgp_1x0_regs/"
regs_f_name = ['Koza 3']
...
regs_method_names = ['CGP(1+4)', 'CGP-1x', 'LGP-1x\n4 Registers\nFixed Length', 'LGP-1x\n4 Registers',
                     'LGP-1x\n2 Registers\nFixed Length', 'LGP-1x\n2 Registers', 'LGP-1x\n0 Registers\nFixed Length',
                     'LGP-1x\n0 Registers']
regs_color_order = ['blue', 'skyblue', 'crimson', 'lightgreen', 'coral', 'saddlebrown', 'orange',
                    'olive']  #, 'cadetblue', 'olive']

fig, axs = plt.subplots(1, 2, figsize=(6.5, 5))
fig.subplots_adjust(hspace=0)
#print(n)
p = 2
regs_boxes = axs[0].boxplot(
    [cgp_base_data['p_fits'][p], cgp_1x_data['p_fits'][p], lgp_1x_4regs_data['p_fits'][0], lgp_1x_data['p_fits'][p],
     lgp_1x_2regs_data['p_fits'][0], lgp_1x_2regv_data['p_fits'][0], lgp_1x_0regs_data['p_fits'][0],
     lgp_1x_0regv_data['p_fits'][0]], showfliers=False, patch_artist=True, vert=False)
...
fig.legend(legend_objects, reversed(regs_method_names), loc='upper center', bbox_to_anchor=(0.5, 0.96),
           ncol=len(legend_objects) / 2)
...
axs[1].fill_between(range(lgp_1x_0_regv_avgs[0].shape[0]), lgp_1x_0_regv_s_m[0], lgp_1x_0_regv_s_p[0],
                    color=color_order[-1], alpha=alpha)
axs[1].fill_between(range(lgp_1x_0_regs_avgs[0].shape[0]), lgp_1x_0_regs_s_m[0], lgp_1x_0_regs_s_p[0],
                    color=color_order[-2], alpha=alpha)
axs[1].fill_between(range(lgp_1x_2_regv_avgs[0].shape[0]), lgp_1x_2_regv_s_m[0], lgp_1x_2_regv_s_p[0],
                    color=color_order[-3], alpha=alpha)
axs[1].fill_between(range(lgp_1x_2_regs_avgs[0].shape[0]), lgp_1x_2_regs_s_m[0], lgp_1x_2_regs_s_p[0],
                    color=color_order[-4], alpha=alpha)
axs[1].fill_between(range(lgp_1x_avgs[2].shape[0]), lgp_1x_s_m[2], cgp_1x_s_p[2], color=color_order[-5], alpha=alpha)
axs[1].fill_between(range(lgp_1x_4_regs_avgs[0].shape[0]), lgp_1x_4_regs_s_m[0], lgp_1x_4_regs_s_p[0],
                    color=color_order[-6], alpha=alpha)
...
alg_out = ','.join(
    map(str, alg_out))  #https://stackoverflow.com/questions/2399112/print-all-items-in-a-list-with-a-delimiter
...
for n in range(len(f_name)):
    ...
    all_meds.append(
        np.round([cgp_base_med, cgp_40_med, cgp_1x_med, lgp_1x_med, cgp_2x_med, lgp_2x_med, cgp_sgx_med, lgp_base_med],
                 5))
...
for p in range(len(f_name)):
    data = [cgp_base_data['p_fits'][p], cgp_40_data['p_fits'][p], cgp_1x_data['p_fits'][p], lgp_1x_data['p_fits'][p],
            cgp_2x_data['p_fits'][p], lgp_2x_data['p_fits'][p], cgp_sgx_data['p_fits'][p], lgp_base_data['p_fits'][p]]
    ...
    for m in range(len(method_names)):
        try:
            ...
            print(f'{method_names[m]} not found')
    print('--------')
...
for n in range(len(f_name)):
    boxes = axs[n].boxplot(
        [cgp_base_data['prop'][n], cgp_40_data['prop'][n], cgp_1x_data['prop'][n], cgp_vlen_data['prop'][n],
         lgp_1x_data['prop'][n], cgp_2x_data['prop'][n], lgp_2x_data['prop'][n], cgp_sgx_data['prop'][n],
         lgp_base_data['prop'][n]], showfliers=False, patch_artist=True)
    boxes = axs[n].boxplot(
        [cgp_base_data['prop'][n], cgp_40_data['prop'][n], cgp_1x_data['prop'][n], lgp_1x_data['prop'][n],
         cgp_2x_data['prop'][n], lgp_2x_data['prop'][n], cgp_sgx_data['prop'][n], lgp_base_data['prop'][n]],
...
for n in range(len(f_name)):
    axs[n].set_yscale('log')
    axs[n].scatter(cgp_base_data['nodes'][n], cgp_base_data['p_fits'][n], label='CGP(1+4)', c=color_order[0],
                   marker='o', s=8)
    axs[n].scatter(cgp_1x_data['nodes'][n], cgp_1x_data['p_fits'][n], label='CGP-OnePoint(40+40)', c=color_order[2],
                   marker='v', s=8)
    #axs[n].scatter(cgp_vlen_data['nodes'][n], cgp_vlen_data['p_fits'][n], label='CGP-OnePoint-Var.Len.', c = color_order[3], marker = 'v',s = 8)
    axs[n].scatter(cgp_2x_data['nodes'][n], cgp_2x_data['p_fits'][n], label='CGP-TwoPoint(40+40)', c=color_order[5],
                   marker='s', s=8)
    axs[n].scatter(cgp_40_data['nodes'][n], cgp_40_data['p_fits'][n], label='CGP(16+64)', c=color_order[1], marker='P',
                   s=8)
    axs[n].scatter(cgp_sgx_data['nodes'][n], cgp_sgx_data['p_fits'][n], label='CGP-Subgraph(40+40)', c=color_order[7],
                   marker='*', s=8)
    #axs[n].scatter(cgp_nx_data['nodes'][n], cgp_nx_data['p_fits'][n], label='CGP-NodeOnePoint(40+40)', c = color_order[8], marker = '<', s = 8)
    axs[n].scatter(lgp_base_data['nodes'][n], lgp_base_data['p_fits'][n], label='LGP-Uniform(40+40)', c=color_order[8],
                   marker='^', s=8)
    axs[n].scatter(lgp_1x_data['nodes'][n], lgp_1x_data['p_fits'][n], label='LGP-OnePoint(40+40)', c=color_order[4],
                   marker='X', s=8)
    axs[n].scatter(lgp_2x_data['nodes'][n], lgp_2x_data['p_fits'][n], label='LGP-TwoPoint(40+40)', c=color_order[6],
                   marker='P', s=8)
...
for n in range(len(f_name)):
    axs[n].set_yscale('log')
    axs[n].scatter(cgp_base_data['prop'][n], cgp_base_data['p_fits'][n], label='CGP(1+4)', c=color_order[0], marker='o',
                   s=8)
    axs[n].scatter(cgp_1x_data['prop'][n], cgp_1x_data['p_fits'][n], label='CGP-OnePoint(40+40)', c=color_order[2],
                   marker='v', s=8)
    axs[n].scatter(cgp_vlen_data['prop'][n], cgp_vlen_data['p_fits'][n], label='CGP-OnePoint-Var. Len.',
                   c=color_order[3], marker='v', s=8)
    axs[n].scatter(cgp_2x_data['prop'][n], cgp_2x_data['p_fits'][n], label='CGP-TwoPoint(40+40)', c=color_order[4],
                   marker='s', s=8)
    axs[n].scatter(cgp_40_data['prop'][n], cgp_40_data['p_fits'][n], label='CGP(16+64)', c=color_order[1], marker='P',
                   s=8)
    axs[n].scatter(cgp_sgx_data['prop'][n], cgp_sgx_data['p_fits'][n], label='CGP-Subgraph(40+40)', c=color_order[7],
                   marker='*', s=8)
    #axs[n].scatter(cgp_nx_data['prop'][n], cgp_nx_data['p_fits'][n], label='CGP-NodeOnePoint(40+40)', c = color_order[8], marker = '<', s = 8)
    axs[n].scatter(lgp_base_data['prop'][n], lgp_base_data['p_fits'][n], label='LGP-Uniform(40+40)', c=color_order[8],
                   marker='^', s=8)
    axs[n].scatter(lgp_1x_data['prop'][n], lgp_1x_data['p_fits'][n], label='LGP-OnePoint(40+40)', c=color_order[4],
                   marker='X', s=8)
    axs[n].scatter(lgp_2x_data['prop'][n], lgp_2x_data['p_fits'][n], label='LGP-TwoPoint(40+40)', c=color_order[6],
                   marker='P', s=8)
...
for n in range(len(f_name)):
    ...
    axs[n].fill_between(range(cgp_vlen_avgs[n].shape[0]), cgp_vlen_s_m[n], cgp_vlen_s_p[n], color=color_order[3],
                        alpha=alpha)
    ...
    axs[n].fill_between(range(cgp_sgx_avgs[n].shape[0]), cgp_sgx_s_m[n], cgp_sgx_s_p[n], color=color_order[7],
                        alpha=alpha)
...
for n in range(len(f_name)):
    ...
    alpha = 0.05
    axs[n].fill_between(range(cgp_drift_avgs[n].shape[0]), cgp_drift_s_m[n], cgp_drift_s_p[n], color=color_order[0],
                        alpha=alpha)
    axs[n].fill_between(range(cgp_40_drift_avgs[n].shape[0]), cgp_40_drift_s_m[n], cgp_40_drift_s_p[n],
                        color=color_order[1], alpha=alpha)
    axs[n].fill_between(range(cgp_1x_drift_avgs[n].shape[0]), cgp_1x_drift_s_m[n], cgp_1x_drift_s_p[n],
                        color=color_order[2], alpha=alpha)
    ...
    axs[n].fill_between(range(lgp_1x_drift_avgs[n].shape[0]), lgp_1x_drift_s_m[n], lgp_1x_drift_s_p[n],
                        color=color_order[4], alpha=alpha)
    axs[n].fill_between(range(cgp_2x_drift_avgs[n].shape[0]), cgp_2x_drift_s_m[n], cgp_2x_drift_s_p[n],
                        color=color_order[5], alpha=alpha)
    axs[n].fill_between(range(lgp_2x_drift_avgs[n].shape[0]), lgp_2x_drift_s_m[n], lgp_2x_drift_s_p[n],
                        color=color_order[6], alpha=alpha)
    axs[n].fill_between(range(cgp_sgx_drift_avgs[n].shape[0]), cgp_sgx_drift_s_m[n], cgp_sgx_drift_s_p[n],
                        color=color_order[7], alpha=alpha)
    axs[n].fill_between(range(lgp_drift_avgs[n].shape[0]), lgp_drift_s_m[n], lgp_drift_s_p[n], color=color_order[8],
                        alpha=0.10)
    axs[n].set_ylim(1e-5, 1)
    axs[n].set_title(f'{f_name[n]}', fontsize=24)
    axs[n].set_ylabel("1-r^2", fontsize=18)
