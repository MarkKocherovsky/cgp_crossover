...
color_order = ['blue', 'royalblue', 'skyblue', 'lightgreen', 'steelblue',
               'mediumseagreen', 'indigo', 'green']  # , 'cadetblue', 'olive']
method_names = ["CGP(1+4)", "CGP(16+64)", "CGP-1x(40+40)", "LGP-1x(40+40)", "CGP-2x(40+40)",
                "LGP-2x(40+40)", "CGP-SGx(40+40)", "LGP-Ux(40+40)"]  # , "CGP-Nx(40+40)", "LGP-Fx(40+40)"]
method_names_long = ["CGP(1+4)", "CGP(16+64)", "CGP-OnePoint(40+40)", "LGP-OnePoint(40+40)", "CGP-TwoPoint(40+40)", "LGP-TwoPoint(40+40)",
                     "CGP-Subgraph(40+40)", "LGP-Uniform(40+40)"]  # , "CGP-NodeOnePoint(40+40)", "LGP-FlattenedOnePoint(40+40)"]
...
for n in range(len(f_name)):
    # print(n)
    boxes = axs[n].boxplot([cgp_base_data['p_fits'][n], cgp_40_data['p_fits'][n], cgp_1x_data['p_fits'][n], lgp_1x_data['p_fits'][n], cgp_2x_data['p_fits']
                           [n], lgp_2x_data['p_fits'][n], cgp_sgx_data['p_fits'][n], lgp_base_data['p_fits'][n]], showfliers=False, patch_artist=True)
    box_list = boxes['boxes']
    axs[n].set_yscale('log')
    for box, color in zip(box_list, color_order):
        box.set_facecolor(color)
    axs[n].set_xticks(list(range(1, len(method_names)+1)),
                      method_names, rotation=0, fontsize=11)
...
fig.legend(legend_objects, method_names_long, fontsize=10, ncol=2,
           bbox_to_anchor=(0.5, 0.965), loc='upper center')
...
for n in range(len(f_name)):
    boxes = axs[n].boxplot([cgp_base_data['prop'][n], cgp_40_data['prop'][n], cgp_1x_data['prop'][n], lgp_1x_data['prop'][n], cgp_2x_data['prop']
                           [n], lgp_2x_data['prop'][n], cgp_sgx_data['prop'][n], lgp_base_data['prop'][n]], showfliers=False, patch_artist=True)
...
for n in range(len(f_name)):
    axs[n].set_yscale('log')
    axs[n].scatter(cgp_base_data['nodes'][n], cgp_base_data['p_fits']
                   [n], label='CGP(1+4)', c=color_order[0], marker='o', s=8)
    axs[n].scatter(cgp_1x_data['nodes'][n], cgp_1x_data['p_fits'][n],
                   label='CGP-OnePoint(40+40)', c=color_order[2], marker='v', s=8)
    axs[n].scatter(cgp_2x_data['nodes'][n], cgp_2x_data['p_fits'][n],
                   label='CGP-TwoPoint(40+40)', c=color_order[4], marker='s', s=8)
    axs[n].scatter(cgp_40_data['nodes'][n], cgp_40_data['p_fits']
                   [n], label='CGP(16+64)', c=color_order[1], marker='P', s=8)
    axs[n].scatter(cgp_sgx_data['nodes'][n], cgp_sgx_data['p_fits'][n],
                   label='CGP-Subgraph(40+40)', c=color_order[6], marker='*', s=8)
    # axs[n].scatter(cgp_nx_data['nodes'][n], cgp_nx_data['p_fits'][n], label='CGP-NodeOnePoint(40+40)', c = color_order[8], marker = '<', s = 8)
    axs[n].scatter(lgp_base_data['nodes'][n], lgp_base_data['p_fits'][n],
                   label='LGP-Uniform(40+40)', c=color_order[7], marker='^', s=8)
    axs[n].scatter(lgp_1x_data['nodes'][n], lgp_1x_data['p_fits'][n],
                   label='LGP-OnePoint(40+40)', c=color_order[3], marker='X', s=8)
    axs[n].scatter(lgp_2x_data['nodes'][n], lgp_2x_data['p_fits'][n],
                   label='LGP-TwoPoint(40+40)', c=color_order[5], marker='P', s=8)
...
for n in range(len(f_name)):
    axs[n].set_yscale('log')
    axs[n].scatter(cgp_base_data['prop'][n], cgp_base_data['p_fits']
                   [n], label='CGP(1+4)', c=color_order[0], marker='o', s=8)
    axs[n].scatter(cgp_1x_data['prop'][n], cgp_1x_data['p_fits'][n],
                   label='CGP-OnePoint(40+40)', c=color_order[2], marker='v', s=8)
    axs[n].scatter(cgp_2x_data['prop'][n], cgp_2x_data['p_fits'][n],
                   label='CGP-TwoPoint(40+40)', c=color_order[4], marker='s', s=8)
    axs[n].scatter(cgp_40_data['prop'][n], cgp_40_data['p_fits']
                   [n], label='CGP(16+64)', c=color_order[1], marker='P', s=8)
    axs[n].scatter(cgp_sgx_data['prop'][n], cgp_sgx_data['p_fits'][n],
                   label='CGP-Subgraph(40+40)', c=color_order[6], marker='*', s=8)
    # axs[n].scatter(cgp_nx_data['prop'][n], cgp_nx_data['p_fits'][n], label='CGP-NodeOnePoint(40+40)', c = color_order[8], marker = '<', s = 8)
    axs[n].scatter(lgp_base_data['prop'][n], lgp_base_data['p_fits'][n],
                   label='LGP-Uniform(40+40)', c=color_order[7], marker='^', s=8)
    axs[n].scatter(lgp_1x_data['prop'][n], lgp_1x_data['p_fits'][n],
                   label='LGP-OnePoint(40+40)', c=color_order[3], marker='X', s=8)
    axs[n].scatter(lgp_2x_data['prop'][n], lgp_2x_data['p_fits'][n],
                   label='LGP-TwoPoint(40+40)', c=color_order[5], marker='P', s=8)
...
cgp_sgx_avgs, cgp_sgx_s_p, cgp_sgx_s_m = get_avg_gens(
    cgp_sgx_data['fit_track'])
...
for n in range(len(f_name)):
    ...
    alpha = 0.05
    axs[n].fill_between(range(cgp_avgs[n].shape[0]), cgp_s_m[n],
                        cgp_s_p[n], color=color_order[0], alpha=alpha)
    axs[n].fill_between(range(cgp_40_avgs[n].shape[0]), cgp_40_s_m[n],
                        cgp_40_s_p[n], color=color_order[1], alpha=alpha)
    axs[n].fill_between(range(cgp_1x_avgs[n].shape[0]), cgp_1x_s_m[n],
                        cgp_1x_s_p[n], color=color_order[2], alpha=alpha)
    axs[n].fill_between(range(lgp_1x_avgs[n].shape[0]), lgp_1x_s_m[n],
                        lgp_1x_s_p[n], color=color_order[3], alpha=alpha)
    axs[n].fill_between(range(cgp_2x_avgs[n].shape[0]), cgp_2x_s_m[n],
                        cgp_2x_s_p[n], color=color_order[4], alpha=alpha)
    axs[n].fill_between(range(lgp_2x_avgs[n].shape[0]), lgp_2x_s_m[n],
                        lgp_2x_s_p[n], color=color_order[5], alpha=alpha)
    axs[n].fill_between(range(cgp_sgx_avgs[n].shape[0]), cgp_sgx_s_m[n],
                        cgp_sgx_s_p[n], color=color_order[6], alpha=alpha)
    axs[n].fill_between(range(lgp_avgs[n].shape[0]), lgp_s_m[n],
                        lgp_s_p[n], color=color_order[7], alpha=0.10)
...
# https://stackoverflow.com/questions/2399112/print-all-items-in-a-list-with-a-delimiter
alg_out = ','.join(map(str, alg_out))
...
for n in range(len(f_name)):
    ...
    all_meds.append(np.round([cgp_base_med, cgp_40_med, cgp_1x_med,
                    lgp_1x_med, cgp_2x_med, lgp_2x_med, cgp_sgx_med, lgp_base_med], 5))
...
for p, ax in enumerate(axs.flat[:len(f_name)]):
    data = [cgp_base_data['p_fits'][p], cgp_40_data['p_fits'][p], cgp_1x_data['p_fits'][p], lgp_1x_data['p_fits']
            [p], cgp_2x_data['p_fits'][p], lgp_2x_data['p_fits'][p], cgp_sgx_data['p_fits'][p], lgp_base_data['p_fits'][p]]
...
lgp_1x_4regs_data = dataDict(get_logs_lgp(
    lgp_1x_4regs_path, f_name=regs_f_name))
lgp_1x_2regs_data = dataDict(get_logs_lgp(
    lgp_1x_2regs_path, f_name=regs_f_name))
lgp_1x_2regv_data = dataDict(get_logs_lgp(
    lgp_1x_2regv_path, f_name=regs_f_name))
lgp_1x_0regs_data = dataDict(get_logs_lgp(
    lgp_1x_0regs_path, f_name=regs_f_name))
lgp_1x_0regv_data = dataDict(get_logs_lgp(
    lgp_1x_0regv_path, f_name=regs_f_name))

regs_method_names = ['CGP(1+4)', 'CGP-1x', 'LGP-1x\n4 Registers\nFixed Length', 'LGP-1x\n4 Registers',
                     'LGP-1x\n2 Registers\nFixed Length', 'LGP-1x\n2 Registers', 'LGP-1x\n0 Registers\nFixed Length', 'LGP-1x\n0 Registers']
regs_color_order = ['blue', 'skyblue', 'lightseagreen', 'lightgreen', 'chartreuse',
                    'springgreen', 'mediumseagreen', 'limegreen']  # , 'cadetblue', 'olive']

fig, axs = plt.subplots(1, 2, figsize=(6.5, 5))
fig.subplots_adjust(hspace=0)
# print(n)
p = 2
regs_boxes = axs[0].boxplot([cgp_base_data['p_fits'][p], cgp_1x_data['p_fits'][p], lgp_1x_4regs_data['p_fits'][0], lgp_1x_data['p_fits'][p], lgp_1x_2regs_data['p_fits']
                            [0], lgp_1x_2regv_data['p_fits'][0], lgp_1x_0regs_data['p_fits'][0], lgp_1x_0regv_data['p_fits'][0]], showfliers=False, patch_artist=True, vert=False)
...
fig.legend(legend_objects, reversed(regs_method_names), loc='upper center',
           bbox_to_anchor=(0.5, 0.96), ncol=len(legend_objects)/2)
axs[0].set_title('Fitness')
# register ablation similarity
cgp_avgs, cgp_s_p, cgp_s_m = get_avg_gens(
    cgp_base_data['average_retention'][:, :, 0, :])
cgp_1x_avgs, cgp_1x_s_p, cgp_1x_s_m = get_avg_gens(
    cgp_1x_data['average_retention'][:, :, 0, :])
lgp_1x_avgs, lgp_1x_s_p, lgp_1x_s_m = get_avg_gens(
    lgp_1x_data['average_retention'][:, :, 0, :])
lgp_1x_4_regs_avgs, lgp_1x_4_regs_s_p, lgp_1x_4_regs_s_m = get_avg_gens(
    lgp_1x_4regs_data['average_retention'][:, :, 0, :])
lgp_1x_2_regs_avgs, lgp_1x_2_regs_s_p, lgp_1x_2_regs_s_m = get_avg_gens(
    lgp_1x_2regs_data['average_retention'][:, :, 0, :])
lgp_1x_0_regs_avgs, lgp_1x_0_regs_s_p, lgp_1x_0_regs_s_m = get_avg_gens(
    lgp_1x_0regs_data['average_retention'][:, :, 0, :])
lgp_1x_2_regv_avgs, lgp_1x_2_regv_s_p, lgp_1x_2_regv_s_m = get_avg_gens(
    lgp_1x_2regv_data['average_retention'][:, :, 0, :])
lgp_1x_0_regv_avgs, lgp_1x_0_regv_s_p, lgp_1x_0_regv_s_m = get_avg_gens(
    lgp_1x_0regv_data['average_retention'][:, :, 0, :])
print(cgp_avgs.shape)
print(cgp_1x_avgs.shape)
print(lgp_1x_avgs.shape)
print(lgp_1x_0_regv_avgs.shape)

axs[1].plot(lgp_1x_0_regv_avgs[0], label=regs_method_names[-1],
            c=regs_color_order[-1])
axs[1].plot(lgp_1x_0_regs_avgs[0], label=regs_method_names[-2],
            c=regs_color_order[-2])
axs[1].plot(lgp_1x_2_regv_avgs[0], label=regs_method_names[-3],
            c=regs_color_order[-3])
axs[1].plot(lgp_1x_2_regs_avgs[0], label=regs_method_names[-4],
            c=regs_color_order[-4])
axs[1].plot(lgp_1x_avgs[2], label=regs_method_names[-5],
            c=regs_color_order[-5])
axs[1].plot(lgp_1x_4_regs_avgs[0], label=regs_method_names[-6],
            c=regs_color_order[-6])
axs[1].plot(cgp_1x_avgs[2], label=regs_method_names[-7],
            c=regs_color_order[-7])
axs[1].plot(cgp_avgs[2], label=regs_method_names[-8], c=regs_color_order[-8])

axs[1].fill_between(range(lgp_1x_0_regv_avgs[0].shape[0]), lgp_1x_0_regv_s_m[0],
                    lgp_1x_0_regv_s_p[0], color=color_order[-1], alpha=alpha)
axs[1].fill_between(range(lgp_1x_0_regs_avgs[0].shape[0]), lgp_1x_0_regs_s_m[0],
                    lgp_1x_0_regs_s_p[0], color=color_order[-2], alpha=alpha)
axs[1].fill_between(range(lgp_1x_2_regv_avgs[0].shape[0]), lgp_1x_2_regv_s_m[0],
                    lgp_1x_2_regv_s_p[0], color=color_order[-3], alpha=alpha)
axs[1].fill_between(range(lgp_1x_2_regs_avgs[0].shape[0]), lgp_1x_2_regs_s_m[0],
                    lgp_1x_2_regs_s_p[0], color=color_order[-4], alpha=alpha)
axs[1].fill_between(range(lgp_1x_avgs[2].shape[0]), lgp_1x_s_m[2],
                    cgp_1x_s_p[2], color=color_order[-5], alpha=alpha)
axs[1].fill_between(range(lgp_1x_4_regs_avgs[0].shape[0]), lgp_1x_4_regs_s_m[0],
                    lgp_1x_4_regs_s_p[0], color=color_order[-6], alpha=alpha)
axs[1].fill_between(range(cgp_1x_avgs[2].shape[0]), cgp_1x_s_m[2],
                    cgp_1x_s_p[2], color=color_order[-7], alpha=alpha)
axs[1].fill_between(range(cgp_avgs[2].shape[0]), cgp_s_m[2],
                    cgp_s_p[2], color=color_order[-8], alpha=alpha)
# axs[1].legend()
...
data = [cgp_base_data['p_fits'][p], cgp_1x_data['p_fits'][p], lgp_1x_4regs_data['p_fits'][0], lgp_1x_data['p_fits'][p],
        lgp_1x_2regs_data['p_fits'][0], lgp_1x_2regv_data['p_fits'][0], lgp_1x_0regs_data['p_fits'][0], lgp_1x_0regv_data['p_fits'][0]]
...
cgp_1x_avgs, cgp_1x_s_p, cgp_1x_s_m = get_avg_gens(
    cgp_1x_data['average_retention'][:, :, 0, :])
cgp_2x_avgs, cgp_2x_s_p, cgp_2x_s_m = get_avg_gens(
    cgp_2x_data['average_retention'][:, :, 0, :])
cgp_40_avgs, cgp_40_s_p, cgp_40_s_m = get_avg_gens(
    cgp_40_data['average_retention'][:, :, 0, :])
cgp_sgx_avgs, cgp_sgx_s_p, cgp_sgx_s_m = get_avg_gens(
    cgp_sgx_data['average_retention'][:, :, 0, :])
lgp_avgs, lgp_s_p, lgp_s_m = get_avg_gens(
    lgp_base_data['average_retention'][:, :, 0, :])
lgp_1x_avgs, lgp_1x_s_p, lgp_1x_s_m = get_avg_gens(
    lgp_1x_data['average_retention'][:, :, 0, :])
lgp_2x_avgs, lgp_2x_s_p, lgp_2x_s_m = get_avg_gens(
    lgp_2x_data['average_retention'][:, :, 0, :])
print(cgp_avgs.shape)
print(cgp_s_p.shape)
print(cgp_s_m.shape)
ratio = 1.5
...
for n, ax in enumerate(axs.flat[:len(f_name)]):
    ...
    alpha = 0.10
    ax.fill_between(range(cgp_avgs[n].shape[0]), cgp_s_m[n],
                    cgp_s_p[n], color=color_order[0], alpha=alpha)
    ax.fill_between(range(cgp_40_avgs[n].shape[0]), cgp_40_s_m[n],
                    cgp_40_s_p[n], color=color_order[1], alpha=alpha)
    ax.fill_between(range(cgp_1x_avgs[n].shape[0]), cgp_1x_s_m[n],
                    cgp_1x_s_p[n], color=color_order[2], alpha=alpha)
    ax.fill_between(range(lgp_1x_avgs[n].shape[0]), lgp_1x_s_m[n],
                    lgp_1x_s_p[n], color=color_order[3], alpha=alpha)
    ax.fill_between(range(cgp_2x_avgs[n].shape[0]), cgp_2x_s_m[n],
                    cgp_2x_s_p[n], color=color_order[4], alpha=alpha)
    ax.fill_between(range(lgp_2x_avgs[n].shape[0]), lgp_2x_s_m[n],
                    lgp_2x_s_p[n], color=color_order[5], alpha=alpha)
    ax.fill_between(range(cgp_sgx_avgs[n].shape[0]), cgp_sgx_s_m[n],
                    cgp_sgx_s_p[n], color=color_order[6], alpha=alpha)
    ax.fill_between(range(lgp_avgs[n].shape[0]), lgp_s_m[n],
                    lgp_s_p[n], color=color_order[7], alpha=0.10)
...
fig.legend(legend_objects, method_names_long, fontsize=10,
           loc='lower right', bbox_to_anchor=(1, 0.05), ncol=2)
...
cgp_avgs, cgp_s_p, cgp_s_m = get_avg_gens(
    cgp_base_data['average_change'][:, :, 0, :])
cgp_1x_avgs, cgp_1x_s_p, cgp_1x_s_m = get_avg_gens(
    cgp_1x_data['average_change'][:, :, 0, :])
cgp_2x_avgs, cgp_2x_s_p, cgp_2x_s_m = get_avg_gens(
    cgp_2x_data['average_change'][:, :, 0, :])
cgp_40_avgs, cgp_40_s_p, cgp_40_s_m = get_avg_gens(
    cgp_40_data['average_change'][:, :, 0, :])
cgp_sgx_avgs, cgp_sgx_s_p, cgp_sgx_s_m = get_avg_gens(
    cgp_sgx_data['average_change'][:, :, 0, :])
lgp_avgs, lgp_s_p, lgp_s_m = get_avg_gens(
    lgp_base_data['average_change'][:, :, 0, :])
lgp_1x_avgs, lgp_1x_s_p, lgp_1x_s_m = get_avg_gens(
    lgp_1x_data['average_change'][:, :, 0, :])
lgp_2x_avgs, lgp_2x_s_p, lgp_2x_s_m = get_avg_gens(
    lgp_2x_data['average_change'][:, :, 0, :])
print(cgp_avgs.shape)
print(cgp_s_p.shape)
print(cgp_s_m.shape)
fig, axs = plt.subplots(len(f_name), 1, figsize=(10, 20))
...
axs[0].legend()
fig.suptitle(
    "Average Change between Best Parents and their Best Children", fontsize=30)
fig.tight_layout(rect=[0, 0, 1, 0.95])  # https://stackoverflow.com/a/45161551
plt.show()
