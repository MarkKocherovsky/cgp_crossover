...
from lgp_data import LgpCollection

warnings.filterwarnings('ignore')

run_name = get_param(1, 'lgp_1x', str)
lgp_c = LgpCollection()
if argv[1] in lgp_c():
    method = lgp_c()[argv[1]]
else:
...
t = get_param(2, 0, int)  # trial
max_g = get_param(3, 100, int)  # generations
max_r = get_param(4, 10, int)  # instructions
max_d = get_param(5, 4, int)  # destinations (other than output)

if max_r < 1:
    print("Number of rules too small, setting to 10")
    max_r = 10

max_p = get_param(6, 50, int)  # parents
max_c = get_param(7, 100, int)  # children
arity = 2  # sources
n_out = 1
...
func, func_name, func_dims = getFunction(int(argv[8]), 2)
n_inp = func_dims
first_body_node = n_inp + n_bias + n_out

p_mut = get_param(9, 0.025)
p_xov = get_param(10, 0.5)
run_name = f'{run_name}{get_param(11, "", str)}'
print(run_name)
fixed_length = get_param(11, False, lambda x: x == '1')
...
sharpness = get_sam_in(noisy_x[:, :, :n_inp], noisy_x[:, :, n_inp:], noisy_y, parents, func, bank, n_inp, max_d, fit,
                       arity)
sharp_in_list = [np.mean(sharpness)]
sharp_in_std = [np.std(sharpness)]

preds = [fitness_evaluator.predict(parent, A, B, n_inp, train_x) for parent, A, B in
